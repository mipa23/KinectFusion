...
bool SurfaceReconstruction::reconstructSurface(CameraParametersPyramid cameraParams, std::shared_ptr<SurfaceMeasurement>& currentFrame, const std::shared_ptr<Volume>& volume, double truncationDistance, int numLevels) {
...
double SurfaceReconstruction::calculateLamdas(Eigen::Vector2i& cameraSpacePoint, CameraParametersPyramid cameraParams) {
...
double SurfaceReconstruction::calculateSDF(double& lambda, Eigen::Vector3d& cameraPosition, double rawDepthValue) {
